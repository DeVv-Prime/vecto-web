# app.py - Prime Web Complete Platform with Interactive Discord Bot (3000+ Lines)
import os
import json
import secrets
import requests
...
# ==================== DISCORD BOT WITH INTERACTIVE MENUS ====================
class PlanSelect(Select):
    def __init__(self, plans, plan_type):
        options = []
        for plan in plans:
            options.append(discord.SelectOption(
                label=f"{plan['name']} - ${plan['price']}/mo",
                description=f"{plan.get('cpu', plan.get('features', 'Premium'))}",
                value=plan['id'],
                emoji=plan['icon']
            ))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if selected_plan:
            embed = discord.Embed(
                title=f"╭╭┈➤ {selected_plan['icon']} {selected_plan['name']}",
                description=f"•〢 **Price:** ${selected_plan['price']}/month\n•〢 **Details:** {selected_plan.get('cpu', selected_plan.get('features', 'Premium'))}",
                color=0x4F46E5
            )
...
class PurchaseModal(discord.ui.Modal, title="˖᯽ Complete Your Purchase ˖᯽"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        order = {
            "id": order_id,
            "items": [{"name": self.plan['name'], "price": self.plan['price'], "quantity": 1}],
            "total": self.plan['price'],
            "customer_name": self.name.value,
        ...
        embed = discord.Embed(
            ...
            description=f"╭╭┈➤ **Order ID:** `{order_id}`\n•〢 **Plan:** {self.plan['name']}\n•〢 **Total:** ${self.plan['price']}\n\n✦ Our team will review your order shortly. You will receive a DM when approved!",
            color=0x10B981
        )
...
class HelpSelect(Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label="🎫 Support", description="Get support", value="support", emoji="🎫"),
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.values[0] == "commands":
            ...
            embed = discord.Embed(
                title="╭╭┈➤ Support",
                description=f"•〢 **Discord Server:** {data['settings']['contact_discord']}\n•〢 **Email:** {data['settings']['contact_email']}\n•〢 **Ticket:** Use `/ticket` to create a support ticket",
                color=0x4F46E5
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            ...
            embed = discord.Embed(
                title="╭╭┈➤ Website",
                description="•〢 Visit our website: [Click Here](http://localhost:5000)\n•〢 Check out our latest plans and offers!",
                color=0x4F46E5
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
...
class PlansView(View):
    def __init__(self):
        super().__init__(timeout=60)
        data = load_db()
        vps_plans = data['plans']['VPS']
        nitro_plans = data['plans']['NITRO']
        self.add_item(PlanSelect(vps_plans, "VPS"))
        self.add_item(PlanSelect(nitro_plans, "NITRO"))
...
def create_bot_instance():
    ...
    async def plans(interaction: discord.Interaction):
        embed = discord.Embed(
            title="˖᯽ Prime Web Hosting Plans ˖᯽",
            description="╭╭┈➤ Select a plan from the dropdown menu below to view details and purchase!\n\n•〢 **VPS Plans** - High-performance virtual servers\n•〢 **Nitro Plans** - Discord Nitro & Boosts",
            color=0x4F46E5
        )
    ...
    async def website(interaction: discord.Interaction):
        embed = discord.Embed(
            title="🌐 Prime Web Website",
            description="•〢 [Click here to visit our website](http://localhost:5000)\n•〢 Check out our latest plans and exclusive offers!",
            color=0x4F46E5
        )
    ...
    async def support(interaction: discord.Interaction):
        data = load_db()
        embed = discord.Embed(
            title="🎫 Prime Web Support",
            description=f"╭╭┈➤ **Discord Server:** {data['settings']['contact_discord']}\n•〢 **Email:** {data['settings']['contact_email']}\n•〢 **Ticket:** Use `/ticket` to create a support ticket",
            color=0x4F46E5
        )
    ...
    async def status_cmd(interaction: discord.Interaction, status: str):
        ...
        embed = discord.Embed(
            ...
            description=f"╭╭┈➤ **New Status:** `{status.upper()}`\n•〢 The bot's presence has been updated successfully!",
            color=0x10B981
        )
        await interaction.response.send_message(embed=embed)
    ...
    async def activity_cmd(interaction: discord.Interaction, activity_type: str, name: str):
        ...
        embed = discord.Embed(
            ...
            description=f"╭╭┈➤ **New Activity:** `{activity_type} {name}`\n•〢 The bot's activity has been updated successfully!",
            color=0x10B981
        )
        await interaction.response.send_message(embed=embed)
    ...
    async def help_cmd(interaction: discord.Interaction):
        embed = discord.Embed(
            title="˖᯽ Prime Web Bot Help ˖᯽",
            description="╭╭┈➤ Welcome to Prime Web Bot! Use the dropdown menu below to explore available options.\n\n•〢 **Quick Commands:**\n  └ /ping - Check latency\n  └ /plans - View plans\n  └ /website - Website link\n  └ /support - Support link\n  └ /ticket - Create ticket\n  └ /close - Close ticket",
            color=0x4F46E5
        )
    ...
    async def ticket(interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description="╭╭┈➤ A support ticket will be created for you.\n•〢 Please describe your issue in the channel.\n•〢 A staff member will assist you shortly.",
            color=0x4F46E5
        )
        
        view = View()
        view.add_item(discord.ui.Button(label="Confirm", style=discord.ButtonStyle.success, custom_id="confirm_ticket"))
        
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
        
        # Store ticket creation intent
        data['tickets'].append({
...
# ==================== WEBSITE TEMPLATE (Atyro Cloud Style) ====================
WEBSITE_TEMPLATE = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes glow {
            0%, 100% { box-shadow: 0 0 5px {{ settings.primary_color }}; }
            50% { box-shadow: 0 0 20px {{ settings.primary_color }}; }
        }
        @keyframes float {
        ...
        .plan-price small { font-size: 0.9rem; font-weight: 400; opacity: 0.7; }
        .plan-features {
            list-style: none;
            margin: 25px 0;
        }
        ...
        .testimonial-card:hover { transform: translateY(-5px); border-color: {{ settings.primary_color }}; }
        .testimonial-content {
            font-style: italic;
            line-height: 1.6;
            margin-bottom: 20px;
        ...
        .testimonial-stars { color: {{ settings.accent_color }}; margin-bottom: 15px; font-size: 1.1rem; }
        ...
        .cta-section p { margin-bottom: 30px; color: {{ settings.text_secondary }}; }
        ...
        .footer-logo img { width: 45px; margin-bottom: 15px; }
        ...
        .social-links a:hover { color: {{ settings.primary_color }}; background: rgba(79,70,229,0.2); transform: translateY(-3px); }
        .footer-bottom {
            text-align: center;
            padding-top: 30px;
            border-top: 1px solid rgba(255,255,255,0.05);
...
<div class="cart-sidebar" id="cartSidebar">
    ...
    <div id="cartTotal" style="font-weight: 800; margin-top: 20px; font-size: 1.2rem;"></div>
...
<div id="paymentModal" class="payment-modal">
    <div class="payment-content">
        ...
        <div style="margin: 20px 0; padding: 15px; background: rgba(255,255,255,0.05); border-radius: 16px;">
            ...
            <img src="{{ payment_config.upi_qr_url }}" style="width:180px; margin:15px 0; border-radius: 12px;">
            {% endif %}
        ...
        <form id="paymentForm">
            <input type="text" id="customerName" placeholder="Your Name" required style="width:100%; padding:14px; margin:10px 0; border-radius:12px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:white;">
            <input type="text" id="discordId" placeholder="Discord ID (for delivery)" required style="width:100%; padding:14px; margin:10px 0; border-radius:12px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:white;">
            <input type="email" id="customerEmail" placeholder="Email (optional)" style="width:100%; padding:14px; margin:10px 0; border-radius:12px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:white;">
            <input type="text" id="transactionId" placeholder="Transaction ID / Screenshot Link" required style="width:100%; padding:14px; margin:10px 0; border-radius:12px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:white;">
            <button type="submit" class="btn-primary" style="width:100%; margin-top:15px;">Submit Order</button>
        </form>
        <button onclick="closePaymentModal()" style="width:100%; margin-top:10px; background:#EF4444; color:white; border:none; padding:14px; border-radius:40px; cursor:pointer;">Cancel</button>
    </div>
</div>
...
<section class="stats-section">
    <div class="container">
        <div class="stats-grid">
            <div class="stat-card"><div class="stat-number">{{ settings.stats_customers }}</div><div class="stat-label">Happy Customers</div></div>
            <div class="stat-card"><div class="stat-number">{{ settings.stats_uptime }}</div><div class="stat-label">Uptime</div></div>
            <div class="stat-card"><div class="stat-number">{{ settings.stats_rating }}</div><div class="stat-label">Rating</div></div>
            <div class="stat-card"><div class="stat-number">{{ settings.stats_servers }}</div><div class="stat-label">Worldwide Servers</div></div>
        </div>
    </div>
</section>
...
<section class="section">
    <div class="container">
        <div class="cta-section">
            ...
            <p>Join thousands of satisfied customers and experience the Prime Web difference today!</p>
        ...
        <div class="footer-grid">
            <div>
                <div class="footer-logo">
                    <img src="{{ settings.logo_url }}" alt="logo" width="45">
...
<script>
    ...
    function updateCartDisplay() {
        ...
        document.getElementById('cartItems').innerHTML = html || '<p style="text-align:center; padding:20px;">Cart is empty</p>';
    ...
    document.querySelectorAll('.feature-card, .plan-card, .testimonial-card, .stat-card').forEach(el => {
        el.style.opacity = '0';
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .login-container p { text-align: center; margin-bottom: 30px; color: #A1A1AA; }
        ...
        .login-container button:hover { transform: translateY(-2px); box-shadow: 0 10px 25px rgba(79,70,229,0.4); }
...
<body>
    <div class="login-container">
        <div class="logo"><img src="https://img.icons8.com/fluency/96/admin-settings-male.png" alt="logo"></div>
        <h1>Prime Web Admin</h1>
        <p>Enter your credentials to access the dashboard</p>
        <form method="post">
            <input type="email" name="email" placeholder="admin@primeweb.com" required>
            <input type="password" name="password" placeholder="••••••••" required>
            <button type="submit"><i class="fas fa-arrow-right"></i> Login to Dashboard</button>
        </form>
        {% if error %}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-approved { background: #10B981; padding: 4px 10px; border-radius: 20px; font-size: 0.7rem; }
        .badge-rejected { background: #EF4444; padding: 4px 10px; border-radius: 20px; font-size: 0.7rem; }
    ...
    <div class="main-content">
        ...
        <div id="dashboard-tab">
            ...
            <div class="card">
                ...
                <p><strong>Status:</strong> <span class="bot-status {% if bot_config.bot_status == 'online' %}status-online{% else %}status-offline{% endif %}">{{ bot_config.bot_status|upper }}</span></p>
                <p><strong>Token:</strong> {% if bot_config.bot_token %}✅ Configured{% else %}❌ Not Set{% endif %}</p>
                <p><strong>Presence:</strong> {{ bot_config.bot_presence|upper }}</p>
                <p><strong>Activity:</strong> {{ bot_config.bot_activity_type }} {{ bot_config.bot_activity }}</p>
                <div style="display: flex; gap: 15px; margin-top: 20px;">
                    <button onclick="startBot()" style="background:#10B981;"><i class="fas fa-play"></i> Start Bot</button>
                    <button onclick="stopBot()" style="background:#EF4444;"><i class="fas fa-stop"></i> Stop Bot</button>
                </div>
                <div id="botMsg"></div>
            </div>
            <div class="card">
                <h3><i class="fas fa-chart-line"></i> Recent Orders</h3>
                {% for order in orders[:5] %}
                <div class="order-item"><strong>{{ order.id[:8] }}</strong> - {{ order.customer_name }} - ${{ order.total }} - <span class="badge-{{ order.status }}">{{ order.status }}</span></div>
                {% else %}
                <p>No orders yet</p>
                {% endfor %}
            </div>
        ...
        <div id="bot-tab" style="display:none;">
            <div class="card">
                <h3><i class="fab fa-discord"></i> Discord Bot Configuration</h3>
                <form id="botForm">
                    <div class="grid-2">
                        <div><label>Bot Token</label><input type="password" name="bot_token" value="{{ bot_config.bot_token }}" placeholder="Discord Bot Token"></div>
                        <div><label>Bot Presence</label><select name="bot_presence"><option value="online" {% if bot_config.bot_presence == 'online' %}selected{% endif %}>🟢 Online</option><option value="idle" {% if bot_config.bot_presence == 'idle' %}selected{% endif %}>🟡 Idle</option><option value="dnd" {% if bot_config.bot_presence == 'dnd' %}selected{% endif %}>🔴 DND</option><option value="invisible" {% if bot_config.bot_presence == 'invisible' %}selected{% endif %}>👻 Invisible</option></select></div>
                        <div><label>Activity Type</label><select name="bot_activity_type"><option value="watching" {% if bot_config.bot_activity_type == 'watching' %}selected{% endif %}>Watching</option><option value="playing" {% if bot_config.bot_activity_type == 'playing' %}selected{% endif %}>Playing</option><option value="listening" {% if bot_config.bot_activity_type == 'listening' %}selected{% endif %}>Listening</option></select></div>
                        <div><label>Activity Name</label><input type="text" name="bot_activity" value="{{ bot_config.bot_activity }}"></div>
                        <div><label>Webhook URL</label><input type="text" name="webhook_url" value="{{ bot_config.webhook_url }}" placeholder="Discord Webhook URL"></div>
                        <div><label>Main Admin ID</label><input type="text" name="main_admin_id" value="{{ admin.main_admin_id or '' }}" placeholder="Your Discord User ID"></div>
                        <div><label>Embed Color</label><input type="color" name="embed_color" value="{{ '#' + bot_config.embed_color|string if bot_config.embed_color else '#4F46E5' }}"></div>
                    </div>
                    <button type="submit"><i class="fas fa-save"></i> Save Bot Settings</button>
                </form>
                <div id="botConfigMsg"></div>
            </div>
            <div class="card">
                <h3><i class="fas fa-terminal"></i> Bot Commands</h3>
                <div class="grid-2">
                    <div><code>/ping</code> - Check latency</div>
                    <div><code>/plans</code> - View plans (interactive)</div>
                    <div><code>/website</code> - Website link</div>
                    <div><code>/support</code> - Support link</div>
                    <div><code>/status</code> - Change status (Admin)</div>
                    <div><code>/activity</code> - Change activity (Admin)</div>
                    <div><code>/ticket</code> - Create ticket</div>
                    <div><code>/close</code> - Close ticket</div>
                    <div><code>/userinfo</code> - User info</div>
                    <div><code>/help</code> - Help menu</div>
                </div>
            </div>
        </div>
        ...
        <div id="plans-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-server"></i> VPS Plans</h3><div id="vpsPlans">{% for plan in plans.VPS %}<div style="padding:12px 0; border-bottom:1px solid rgba(255,255,255,0.1);"><strong>{{ plan.icon }} {{ plan.name }}</strong> - ${{ plan.price }}/mo<button onclick="editPlan('VPS',{{ loop.index0 }})" style="background:#F59E0B; margin-left:15px; padding:6px 16px;">Edit</button></div>{% endfor %}</div><button onclick="addPlan('VPS')"><i class="fas fa-plus"></i> Add VPS Plan</button></div>
            <div class="card"><h3><i class="fab fa-discord"></i> Nitro Plans</h3><div id="nitroPlans">{% for plan in plans.NITRO %}<div style="padding:12px 0; border-bottom:1px solid rgba(255,255,255,0.1);"><strong>{{ plan.icon }} {{ plan.name }}</strong> - ${{ plan.price }}/mo<button onclick="editPlan('NITRO',{{ loop.index0 }})" style="background:#F59E0B; margin-left:15px; padding:6px 16px;">Edit</button></div>{% endfor %}</div><button onclick="addPlan('NITRO')"><i class="fas fa-plus"></i> Add Nitro Plan</button></div>
        </div>
        
        <!-- Payment Tab -->
        <div id="payment-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-credit-card"></i> Payment Configuration</h3><form id="paymentForm"><div class="grid-2"><div><label>UPI ID</label><input type="text" name="upi_id" value="{{ payment_config.upi_id }}"><label>UPI QR URL</label><input type="text" name="upi_qr_url" value="{{ payment_config.upi_qr_url }}"></div><div><label>Bank Name</label><input type="text" name="bank_name" value="{{ payment_config.bank_name }}"><label>Account Number</label><input type="text" name="account_number" value="{{ payment_config.account_number }}"><label>IFSC Code</label><input type="text" name="ifsc_code" value="{{ payment_config.ifsc_code }}"></div></div><label>Instructions</label><textarea name="payment_instructions" rows="3">{{ payment_config.payment_instructions }}</textarea><label>Auto Approve</label><select name="auto_approve"><option value="True" {% if payment_config.auto_approve %}selected{% endif %}>Yes</option><option value="False" {% if not payment_config.auto_approve %}selected{% endif %}>No</option></select><button type="submit"><i class="fas fa-save"></i> Save Payment Settings</button></form><div id="paymentMsg"></div></div>
        </div>
        
        <!-- Orders Tab -->
        <div id="orders-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-clock"></i> Pending Orders</h3><div id="pendingOrders">{% for order in orders if order.status == 'pending' %}<div class="order-item"><strong>🆔 {{ order.id }}</strong> <span class="badge-pending">PENDING</span><p>👤 {{ order.customer_name }} | Discord: {{ order.discord_id }}</p><p>💰 ${{ order.total }} | 📅 {{ order.date[:19] if order.date else 'Just now' }}</p><div><button onclick="approveOrder('{{ order.id }}')" style="background:#10B981; padding:6px 16px;">✅ Approve</button><button onclick="rejectOrder('{{ order.id }}')" style="background:#EF4444; padding:6px 16px; margin-left:10px;">❌ Reject</button><button onclick="contactCustomer('{{ order.discord_id }}')" style="background:#5865F2; padding:6px 16px; margin-left:10px;">💬 Contact</button></div></div>{% else %}<p>No pending orders</p>{% endfor %}</div></div>
            <div class="card"><h3><i class="fas fa-history"></i> Order History</h3><div id="allOrders">{% for order in orders %}<div class="order-item"><strong>{{ order.id[:12] }}</strong> - {{ order.customer_name }} - ${{ order.total }} - <span class="badge-{{ order.status }}">{{ order.status }}</span><br><small>{{ order.date[:19] if order.date else 'Unknown' }}</small></div>{% else %}<p>No orders</p>{% endfor %}</div></div>
        </div>
        
        <!-- Settings Tab -->
        <div id="settings-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-palette"></i> Website Appearance</h3><form id="settingsForm"><div class="grid-2"><div><label>Website Name</label><input type="text" name="website_name" value="{{ settings.website_name }}"><label>Logo URL</label><input type="text" name="logo_url" value="{{ settings.logo_url }}"><label>Primary Color</label><input type="color" name="primary_color" value="{{ settings.primary_color }}"></div><div><label>Hero Title</label><input type="text" name="hero_title" value="{{ settings.hero_title }}"><label>Hero Subtitle</label><input type="text" name="hero_subtitle" value="{{ settings.hero_subtitle }}"><label>Hero Button Text</label><input type="text" name="hero_button_text" value="{{ settings.hero_button_text }}"></div></div><label>Contact Email</label><input type="email" name="contact_email" value="{{ settings.contact_email }}"><label>Discord Invite</label><input type="text" name="contact_discord" value="{{ settings.contact_discord }}"><button type="submit"><i class="fas fa-save"></i> Save Settings</button></form><div id="settingsMsg"></div></div>
            <div class="card"><h3><i class="fas fa-chart-simple"></i> Statistics</h3><form id="statsForm"><div class="grid-2"><div><label>Customers Count</label><input type="text" name="stats_customers" value="{{ settings.stats_customers }}"></div><div><label>Uptime</label><input type="text" name="stats_uptime" value="{{ settings.stats_uptime }}"></div><div><label>Rating</label><input type="text" name="stats_rating" value="{{ settings.stats_rating }}"></div><div><label>Servers</label><input type="text" name="stats_servers" value="{{ settings.stats_servers }}"></div></div><button type="submit"><i class="fas fa-save"></i> Save Statistics</button></form><div id="statsMsg"></div></div>
        </div>
        
        <!-- Features Tab -->
        <div id="features-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-star"></i> Features Section</h3><form id="featuresTitleForm"><label>Features Title</label><input type="text" name="features_title" value="{{ settings.features_title }}"><label>Features Subtitle</label><input type="text" name="features_subtitle" value="{{ settings.features_subtitle }}"><button type="submit"><i class="fas fa-save"></i> Save Title</button></form></div>
            <div class="card"><h3><i class="fas fa-list"></i> Manage Features</h3><div id="featuresList">{% for f in features %}<div style="padding:12px 0; border-bottom:1px solid rgba(255,255,255,0.1);"><strong>{{ f.icon }} {{ f.title }}</strong><p>{{ f.description }}</p><button onclick="editFeature({{ loop.index0 }})" style="background:#F59E0B; padding:6px 16px;">Edit</button><button onclick="deleteFeature({{ loop.index0 }})" style="background:#EF4444; padding:6px 16px; margin-left:10px;">Delete</button></div>{% endfor %}</div><button onclick="addFeature()"><i class="fas fa-plus"></i> Add Feature</button></div>
        </div>
        
        <!-- Testimonials Tab -->
        <div id="testimonials-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-comment"></i> Testimonials Settings</h3><form id="testimonialsForm"><label>Show Testimonials</label><select name="testimonials_enabled"><option value="True" {% if settings.testimonials_enabled %}selected{% endif %}>Yes</option><option value="False">No</option></select><button type="submit"><i class="fas fa-save"></i> Save</button></form></div>
            <div class="card"><h3><i class="fas fa-users"></i> Manage Testimonials</h3><div id="testimonialsList">{% for t in testimonials %}<div style="padding:12px 0; border-bottom:1px solid rgba(255,255,255,0.1);"><strong>{{ t.name }}</strong> ({{ t.role }})<p>"{{ t.content }}"</p><div class="testimonial-stars">{% for i in range(t.rating) %}⭐{% endfor %}</div><button onclick="editTestimonial({{ loop.index0 }})" style="background:#F59E0B; padding:6px 16px;">Edit</button><button onclick="deleteTestimonial({{ loop.index0 }})" style="background:#EF4444; padding:6px 16px; margin-left:10px;">Delete</button></div>{% endfor %}</div><button onclick="addTestimonial()"><i class="fas fa-plus"></i> Add Testimonial</button></div>
        </div>
        
        <!-- Password Tab -->
        <div id="password-tab" style="display:none;">
            <div class="card"><h3><i class="fas fa-shield-alt"></i> Change Admin Password</h3><form id="passwordForm"><input type="password" name="current_password" placeholder="Current Password"><input type="password" name="new_password" placeholder="New Password"><input type="password" name="confirm_password" placeholder="Confirm Password"><button type="submit"><i class="fas fa-key"></i> Change Password</button></form><div id="passwordMsg"></div></div>
        </div>
    </div>
</div>
...
<script>
    ...
    document.getElementById('botForm')?.addEventListener('submit', (e) => { e.preventDefault(); const data = Object.fromEntries(new FormData(e.target)); data.main_admin_id = parseInt(data.main_admin_id) || null; data.embed_color = parseInt(data.embed_color.replace('#', ''), 16) || 0x4F46E5; postData('/admin/api/save-bot', data, document.getElementById('botConfigMsg')); });
    document.getElementById('paymentForm')?.addEventListener('submit', (e) => { e.preventDefault(); const data = Object.fromEntries(new FormData(e.target)); data.auto_approve = data.auto_approve === 'True'; postData('/admin/api/save-payment', data, document.getElementById('paymentMsg')); });
    ...
    document.getElementById('featuresTitleForm')?.addEventListener('submit', (e) => { e.preventDefault(); postData('/admin/api/save-features-title', Object.fromEntries(new FormData(e.target))); });
    document.getElementById('testimonialsForm')?.addEventListener('submit', (e) => { e.preventDefault(); postData('/admin/api/save-testimonials-settings', Object.fromEntries(new FormData(e.target))); });
...
def place_order():
    ...
    order_id = secrets.token_hex(8).upper()
    order = {
        "id": order_id, "items": request.json.get('items', []), "total": request.json.get('total', 0),
        "customer_name": request.json.get('customer_name'), "discord_id": request.json.get('discord_id'),
        "email": request.json.get('email'), "transaction_id": request.json.get('transaction_id'),
    ...
    if webhook_url:
        ...
        send_discord_webhook(webhook_url, "🛒 New Order - Pending Approval", description, 0xF59E0B)
    
    return jsonify({"success": True, "order_id": order_id})

def send_discord_webhook(webhook_url, title, description, color=0x4F46E5):
    try:
        embed = {"title": title, "description": description, "color": color, "timestamp": datetime.now().isoformat()}
...
def save_settings():
    if not session.get('admin_logged_in'): return jsonify({"error": "Unauthorized"}), 401
    data = load_db()
    for key in ['website_name', 'logo_url', 'primary_color', 'hero_title', 'hero_subtitle', 'hero_button_text', 'contact_email', 'contact_discord']:
...
if __name__ == '__main__':
    ...
    print("💬 Bot Commands: /plans (interactive dropdown), /help, /ticket, /ping")
    print("=" * 60)
